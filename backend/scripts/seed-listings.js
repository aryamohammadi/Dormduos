 ...
 * 
 * Usage:
 *   Local: MONGODB_URI="your-uri" node scripts/seed-listings.js
 *   Railway: railway run node scripts/seed-listings.js
 */
...
// Get MongoDB URI - prioritize MONGO_URL (Railway) over MONGODB_URI (Atlas)
// Also allow command line argument
const MONGODB_URI = process.argv[2] || process.env.MONGO_URL || process.env.MONGODB_URI;

if (!MONGODB_URI) {
  console.error('❌ MONGODB_URI or MONGO_URL not found');
  console.error('');
  console.error('Usage options:');
  console.error('  1. Set environment variable: MONGODB_URI="your-uri" node scripts/seed-listings.js');
  console.error('  2. Pass as argument: node scripts/seed-listings.js "your-uri"');
  console.error('  3. Set in backend/.env file: MONGODB_URI=your-uri');
  console.error('  4. On Railway: railway run node scripts/seed-listings.js');
  process.exit(1);
}

// Add database name if not in URI (for MongoDB Atlas)
let finalURI = MONGODB_URI;
if (MONGODB_URI.includes('mongodb+srv://') && !MONGODB_URI.includes('/ucrhousing') && !MONGODB_URI.includes('?') && !MONGODB_URI.endsWith('/')) {
  finalURI = MONGODB_URI.endsWith('/') ? MONGODB_URI + 'ucrhousing' : MONGODB_URI + '/ucrhousing';
...
async function seedListings() {
  try {
    ...
    console.log('📍 Using URI:', finalURI.replace(/:[^:@]+@/, ':****@'));
